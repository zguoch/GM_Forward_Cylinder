function cal_mag_trans_new_ext(str_open,I0_ang,D0_ang,trans_para,DirRes,SavFor,ext_para)

%函数说明： 磁场转换函数
%By KICIOLLO 
%Based on the routines: MMTrans

%输入参数： str_open       网格数据filename或者是pathname都可以
%           I0_ang D0_ang  地磁场（正常场）倾角及偏角
%           trans_para     参数，用以确定是进行哪种变换
%           = 1   Za -> Hax
%           = 1   Za -> Hay
%           （由Za转换至Hax Hay的部分，是空的，此处注意，所以参数只可以是 2 或 3）
...
% 开始进行转换操作
if trans_para == 1
%  Delta2HaxHayZa(Mat,Dx,Dy,I0_ang,D0_ang,Per);
%  % Cut the resultant matrices to original size
%  FulNamHax=[Dir FilSep Nam '_Hax'];
%  load X；
%  X=RecMat(X,FulNamHax,RowGap,ColGap,Str,SavFor);
%  delete('X.mat');
%  FulNamHay=[Dir FilSep Nam '_Hay'];
%  load Y；
%  Y=RecMat(Y,FulNamHay,RowGap,ColGap,Str,SavFor);
%  delete('Y.mat');
%  FulNamZa=[Dir FilSep Nam '_Za'];
%  load Z；
%  Z=RecMat(Z,FulNamZa,RowGap,ColGap,Str,SavFor);
%  delete('Z.mat');

elseif trans_para == 2
  Delta2HaxHayZa(Mat,Dx,Dy,I0_ang,D0_ang,Per,ext_para);
  % Cut the resultant matrices to original size
  FulNamHax=[DirRes FilSep Nam '_Hax'];
  load X;
  X=RecMat(X,FulNamHax,RowGap,ColGap,Str,SavFor);
%  delete('X.mat');
  FulNamHay=[DirRes FilSep Nam '_Hay'];
  load Y;
  Y=RecMat(Y,FulNamHay,RowGap,ColGap,Str,SavFor);
%  delete('Y.mat');
  FulNamZa=[DirRes FilSep Nam '_Za'];
  load Z;
  Z=RecMat(Z,FulNamZa,RowGap,ColGap,Str,SavFor);
%  delete('Z.mat');
%  delete('T.mat');
...
elseif trans_para == 3
  Delta2HaxHayZa(Mat,Dx,Dy,I0_ang,D0_ang,Per,ext_para);
  FulNamTa=[DirRes FilSep Nam '_Ta'];
  load X;
  load Y;
  load Z;
  T = sqrt(X.*X + Y.*Y + Z.*Z);
  
  T=RecMat(T,FulNamTa,RowGap,ColGap,Str,SavFor);
  delete('X.mat');
  delete('Y.mat');
  delete('Z.mat');
  save T T;
...
elseif trans_para == 4
  ...
  T=RecMat(T,FulNamTa,RowGap,ColGap,Str,SavFor);
  clear T;
  % Save the final mat file in the results directory 
%  eval(['save' ' ' FulNamHax '.mat' ' ' 'T']); 
...
end % function cal_mag_trans_new
