% %%
function [ imf_matrix residue YuZhi] = bemd( input_image_d )
...
% The process of Sifting  筛分的过程
% 设置分解阶次 DEC_NUM
DEC_NUM=2; % 这里的阶次数表示不包括剩余分量
while(k<(DEC_NUM+1))    % 当SD确定后，相应的分解阶次K也就确定了，当然也可以设置其分解阶次k,但必须有k<=K,可通过改变k确定K.
    [imf_temp residue_temp Yuzhi] = sift(h_func); 
    imf_matrix(:,:,k) = imf_temp; %#ok<AGROW>
    k = k+1;
    h_func = residue_temp;
    residue(:,:,k)=h_func;% 把筛选一次后剩余图像进行存储
end
YuZhi=Yuzhi;
...
residue(:,:,k) = residue_temp;%把筛分结束后的剩余信号进行存储
...
function [ h_imf residue Yuzhi] = sift( input_image )
...
while(1)
    ...
    % Finding the extrema in the 2D signal  查找二维信号极值点（极大值和极小值）
    
    [zmax imax zmin imin] = extrema2(input_image_temp);
    [xmax ymax] = ind2sub(size(input_image_temp),imax);%把数值中元素索引值转换为该元素在数值中对应的下标，索引值为imax，返回值为索引值为imax的元素在该二维数组内的行列坐标 xmax为行 ymax为列
    [xmin ymin] = ind2sub(size(input_image_temp),imin);

    % Interpolating the extrema to get the extrema suraces 利用极值插值计算上下包络面

    [zmaxgrid , ~, ~] = gridfit(ymax,xmax,zmax,y,x);%上包络
    [zmingrid, ~, ~] = gridfit(ymin,xmin,zmin,y,x);%下包络
    
    % Averaging the extrema to get the Zavg surface  计算上下包络面均值
    
    zavggrid = (zmaxgrid + zmingrid)/2;
    
    % Computation of the h_imf (IMF for the 'h' input) 剩余值中减去均值
    
    h_imf = input_image_temp - zavggrid;

    % Computing IMF cost  BEMD分解终止准则
    ...
    % Checking the IMF accuracy  当标准偏差小于阈值时，则分解被终止；否则剩余值减去均值重新作为原始信号重新用于筛分
    Yuzhi=0.0000001;
    if cost< Yuzhi % 一般取值范围为0.1~0.3，默认值为0.2  设置的数值越小，分解得到的图形细节越明显，反之，细节不明显
        break;
    else
        input_image_temp = h_imf;
    end
    % 判断极值点的个数，当个数小于2个时，终止程序跳出
    
    
end

% Computation of the Residue after IMF computation   IMF计算结束后计算剩余量
...
% Vector input?
Nt = numel(x);%返回数值x 元素的个数
if Nt ~= length(x)
 error('Entry must be a vector.')
end

% NaN's:  NAN是not a number的缩写
inan = find(isnan(x));%isnan函数的作用是如果x中有非数（NaNs，比如0/0），则取1，否则取0，这样isnan(x)得到的就是只含0和1的向量
indx = 1:Nt;
if ~isempty(inan) %判断是非空集
 indx(inan) = [];%赋予空集
 x(inan) = []; 
 Nt = length(x);
end

% Difference between subsequent elements: 求相邻元素的微分（差商）
dx = diff(x);

% Is an horizontal line?  dx中任何一个元素为非零，any(dx)返回逻辑词1
if ~any(dx)
 return
end
...
%   The user should be aware of these limitations.列、行、对角线
%
...
M = size(xy);
if length(M) ~= 2
 error('Entry must be a matrix.') %矩阵
end
N = M(2);
M = M(1);

% Search peaks through columns: 通过列搜索极值
...
% Peaks through columns or rows. 通过列、行
...
% Peaks through diagonals (down-up A=-1) 通过对角线
...
% Carlos Adri醤 Vargas Aguilera. nubeobscura@hotmail.com
...
%
%          DEFAULT: 0.20  停止准则 一般取值范围0.2~0.3
%
%
...
params.tilesize = inf;
params.overlap = 0.20;%可进行修改 0.02 0.01
params.mask = []; 
params.autoscale = 'on';
params.xscale = 1;
params.yscale = 1;
...
% ============================================
% subfunction - parse_pv_pairs    子函数
% ============================================
