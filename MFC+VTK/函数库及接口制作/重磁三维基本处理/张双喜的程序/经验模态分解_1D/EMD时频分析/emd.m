...
% 关于emd函数：本函数用来对局域波进行经验模态分解

%函数语法为：
% 输入有:                x : 该变量定义为一个行向量，即待处理时间序列信号
...
%
% 输出为:                imf : 为一个矩阵，矩阵第一行为一个内蕴模态函数，最后一行为残余分量
%                       ort : 正交性的索引值
...
% 调用函数：init，stop_EMD，stop_sifting_fixe，stop_sifting_fixe_h，stop_sifting
function [imf,ort,nbits] = emd(varargin)
    %调用初始化函数init，对各输入参数进行初始化
    ...
    %判断display_sifting是否为真，如为真，则获得图形句柄
    if display_sifting
        fig_h = figure;
    end

    %main loop : requires at least 3 extrema to proceed
    %主循环，至少需要3个极值点才能够执行此循环，stop_EMD（）函数用来判断信号序列极值点数目是否大于等于3；已分解模态函数个数是否已达到设
    %定最大值；mask？
...
% 关于stop_EMD函数，本函数用来判断极值点是否大于等于3，如不够，则分解停止
%函数语法为：
% 输入有:                
%                       r : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       MODE_COMPLEX:
%                       该变量用来指示待分解序列是否为复数序列，如为复数序列则值取1，否则值取0，其初值在init函数中定义
%                       ndirs: 为一个向量，记录每分解得到一个内蕴模式函数需要的迭代次数
%
% 输出为:                
%                       stop : 分解终止条件
%                       
...
function stop = stop_EMD(r,MODE_COMPLEX,ndirs)
    ...
    if MODE_COMPLEX
        for k = 1:ndirs
            phi = (k-1)*pi/ndirs;
            %调用extr函数，记录极小值点序号，和极大值点序号到相应向量中
...
% 关于mean_and_amplitude函数，本函数用来计算极大值与极小值对信号序列的包络的均值
%函数语法为：
% 输入有:                
%                       m : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       t : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       INTERP : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       MODE_COMPLEX:
%                       该变量用来指示待分解序列是否为复数序列，如为复数序列则值取1，否则值取0，其初值在init函数中定义
%                       ndirs: 为一个向量，记录每分解得到一个内蕴模式函数需要的迭代次数
%
    ...
    if MODE_COMPLEX
        switch MODE_COMPLEX
            case 1
                ...
                if nargout > 3
                    amp =  2./mean(abs(envmax-envmin),1);
                end
            case 2
                for k = 1:ndirs
                    phi = (k-1)*pi/ndirs;
...
%函数语法为：
% 输入有:                
%                       x : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       imf : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%
% 输出为:                
%                       ort :  
%
...
%---------------------------------------------------------------------------------------------------
% 关于init函数，本函数用来初始化输入变量
%函数语法为：
% 输入有:  
%                       varargin：输入参数向量 
%
% 输出为:                
%                       x : 该变量定义为一个行向量，即待处理时间序列信号
%                       t : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       sd : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       sd2 : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       tol : 该变量定义为一个行向量，即待处理时间序列信号残余分量

%                       MODE_COMPLEX:
%                       该变量用来指示待分解序列是否为复数序列，如为复数序列则值取1，否则值取0
%                       ndirs : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       display_sifting : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       sdt : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       sd2t : 该变量定义为一个行向量，即待处理时间序列信号残余分量

%                       r : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       imf : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       k : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       nbit : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       NbIt : 该变量定义为一个行向量，即待处理时间序列信号残余分量

%                       MAXITERATIONS : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       FIXE : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       FIXE_H : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       MAXMODES : 该变量定义为一个行向量，即待处理时间序列信号残余分量
%                       INTERP : 该变量定义为一个行向量，即待处理时间序列信号残余分量

%                       mask: 为一个向量，记录每分解得到一个内蕴模式函数需要的迭代次数
%
% 调用函数 :             
%                       extr ()：统计输入序列中极值点和过0点数目
